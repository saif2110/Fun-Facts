...
enum IPA:String {
    case OneWeekPro = "FactsOneMonthPro"
...
class InAppPurchases: UIViewController {
    ...
    override func viewDidLoad() {
       ...
       //startIndicator(selfo: self, UIView: self.view)
        ...
        customLabelBanner(vw: WeekLowerLabel)
        upperView(vw: WeekUpperView)
        upperView(vw: WeekUpperLabel)
        
        customLabelBanner(vw: YearLowerLabel)
        ...
        Purchases.shared.offerings { (offerings, error) in
            ...
            if let offerings = offerings {
                ...
                let priceone = offerings[IPA.OneWeekPro.rawValue]?.monthly?.localizedPriceString
                
                let pricetwo = offerings[IPA.OneYearPro.rawValue]?.lifetime?.localizedPriceString
                
                
                self.WeekLowerLabel.text = priceone //self.PriceMessage(price: priceone ?? "$0.49" , save: "Save 5%")
